...
import { PROJECT_PHASES, TODAY } from '../../data/constants';
...
const getStatusStyle = (status) => {
    if (status === 'On Track' || status === 'Done') {
...
// Get phase label from phase id
const getPhaseLabel = (phaseId) => {
    const phase = PROJECT_PHASES.find(p => p.id === phaseId);
    return phase ? phase.label : phaseId.replace('_', ' ');
};

// Get phase styling from phase id
const getPhaseStyle = (phaseId) => {
    const phase = PROJECT_PHASES.find(p => p.id === phaseId);
    return phase || PROJECT_PHASES[0];
};

const DailyOverviewSlideOver = ({ isOpen, onClose, projects }) => {
    ...
    const projectsWithTodayPhase = useMemo(() => {
        return projects.map(project => {
            // Find the phase that includes today
                ...
                <div>
                    ...
                    {projectsWithTodayPhase.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {projectsWithTodayPhase.map(project => {
                                const ProjectIcon = ICON_MAP[project.icon] || Database;
                                const phaseStyle = project.todayPhase
                                    ? getPhaseStyle(project.todayPhase.id)
                                    : null;
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-1.5 mb-2">
                                            <span className={`text-[7px] font-black px-1 py-0.5 rounded border ${getPriorityStyle(project.priority)}`}>
                                        ...
                                        {project.todayPhase ? (
                                            <div className="pt-2 border-t border-[#1e293b]">
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className={`text-[8px] font-black uppercase tracking-wider ${phaseStyle?.text || 'text-slate-400'}`}>
                                                        {getPhaseLabel(project.todayPhase.id)}
                                                    </span>
                                                ...
                                                <div className="w-full h-1 bg-[#1e293b] rounded-full overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full ${phaseStyle?.accent || 'bg-slate-500'}`}
