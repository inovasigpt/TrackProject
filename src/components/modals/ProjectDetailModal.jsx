...
import {
    ...
    Info, Layers, FileText, StickyNote
} from 'lucide-react';
...
const getStatusStyle = (status) => {
    try {
        ...
        if (saved) {
            ...
            const found = statuses.find(s => s.label === status);
            if (found) {
                const colorMap = {
...
export const ProjectDetailContent = ({ project, activeTab, setActiveTab }) => {
    const picsList = project.pics?.length > 0 ? project.pics : project.pic ? [project.pic] : [];
                            ...
                            <div className="space-y-2">
                                {picsList.map((pic, index) => (
                                    <div key={index} className="flex items-center gap-3 bg-[#020617]/50 p-3 rounded-lg border border-[#1e293b]">
                                        <img className="w-10 h-10 rounded-lg border border-purple-500/30 object-cover" src={pic.avatar} alt={pic.name} />
                                        <div className="flex-1">
                                            <p className="text-white text-sm font-bold">{pic.name}</p>
                                            <p className="text-purple-400 text-[10px] font-black uppercase tracking-widest">{pic.role}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-2">
                                {project.phases.map((phase) => (
                                    <div key={phase.id} className="flex items-center justify-between p-3 bg-[#020617]/50 rounded-lg border border-[#1e293b]">
                                        <span className="text-[11px] text-white uppercase font-bold">{phase.id.replace('_', ' ')}</span>
                                        <div className="flex items-center gap-3">
                                            <span className="text-[9px] font-mono text-slate-500">{phase.startDate} → {phase.endDate}</span>
...
const ProjectDetailModal = ({ project, onClose, onEdit, onArchive, isArchived = false }) => {
