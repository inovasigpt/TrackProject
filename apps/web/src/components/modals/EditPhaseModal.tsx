...
interface EditPhaseModalProps {
    data: PhaseData | null;
    onClose: () => void;
    onSave: (projectId: string, phaseId: string, formData: PhaseFormData) => void;
}

const EditPhaseModal: React.FC<EditPhaseModalProps> = ({ data, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (data?.phase) {
            setFormData({
                startDate: data.phase.startDate ? new Date(data.phase.startDate).toISOString().split('T')[0] : '',
                endDate: data.phase.endDate ? new Date(data.phase.endDate).toISOString().split('T')[0] : '',
